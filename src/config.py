...
SMPL_FACE_PATH = osp.join(base_dir, 'src/tf_smpl', 'smpl_faces.npy')

# Default pred-trained model path for the demo.
PRETRAINED_MODEL = osp.join(model_dir, 'model.ckpt-1931604') # demo model load path
...
DATA_DIR = osp.join(base_dir, 'tf_records')

flags.DEFINE_string('data_dir', DATA_DIR, 'tf_records dir')
...
# Hyper parameters:
flags.DEFINE_float('e_lr', 1e-5, 'Encoder learning rate')
flags.DEFINE_float('d_lr', 1e-4, 'Adversarial prior learning rate')
...
flags.DEFINE_float('e_3d_weight', 60, 'weight on E_3d')
...
def prepare_dirs(config, prefix=['HMR']):
    ...
    if config.load_path:
        ...
        config.model_dir = osp.join(config.logs_dir, save_name)

    for path in [config.logs_dir, config.model_dir]:
